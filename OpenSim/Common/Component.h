...
template <class T> friend class ComponentMeasure;
  ...
  /** Single call to construct the underlying infastructure of a Component, which
	...
	*/
	...
	/**
    * Construct an Input (socket) for this Component's dependence on an Output
    * signal.  It is a placeholder for the Output and its type and enables the
    * Component to automatically traverse its dependencies and provide a
    * meaningful message if the provided Output is incompatible or
    * non-existant. The also specifies at what stage the output must be valid
    * for the component to consume it as an input.
    * if the Output's dependsOnStage is above the Input's requiredAtStage, an
    * Exception is thrown because the output cannot satisfy the Input's
    * requirement.
	*/
	template <typename T>
...
private:
    ...
    //hookup to other components in order to function. For example, a Joint
    //needs a parent body in order to join its owning body to the model. A
    //Connector formalizes this dependendency. The Component is inoperable
    //until the Connector is satisfied. Connectors are not to be confused with
    //subcomponents, with the key difference being that a subcomponent is part
    //of and owned by the component, whereas a Connector is a requirement or a
    //"slot" that must be satisfied by the time the system is ready to
    //simulate. 
	//Connectors are resolved in Component's connect().
    //constructStructuralDependencies is a series of calls to
    //constructConnector() which adds a component by name and type to a
    //dependency Connectors table.
    ...
    //connector, an input specifies the required flow of data into the
    //component.
	//@see Component::Input
	virtual void constructInputs() {}

    //Construct the table of Outputs provided by this component. An Output is a
    //data signal generated by this Component. It can be any response or
    //calculation made by the Component as a function of the state.
    //Specifically, an Output is a redirect to a method on the Component and a
    ...
    // Reset by clearing underlying system indices, disconnecting connectors
    // and creating a fresh connectorsTable.
    void reset() {
...
protected:
    //Derived Components must create concrete StateVariables to expose their
    //state variables. When exposing state variables allocated by the
    //underlying Simbody component (MobilizedBody, Constraint, Force, etc...)
    //use its interface to implement the virtual methods below. Otherwise, if
    //the Component is adding its own state variables using the
    //addStateVariable() helper, then an 
    //AddedStateVariable implements the interface and automatically handles
	...
	public:
        ...
        //Concrete Components implement how the state variable value is
        //evaluated
	...
	private:
        ...
        // Identify which subsystem this state variable belongs to, which
        // should be determined and set at creation time
...
private:
    ...
    // Underlying SimTK custom measure ComponentMeasure, which implements the
    // realizations in the subsystem by calling private concrete methods on the
    // Component. Every model component has one of these, allocated in its
    // addToSystem() method, and placed in the System's default subsystem.
    ...
    // bookkeeping of component variables (state variables and cache entries)
    // with their index in the computational system. The earliest time we have
    // a valid index is when we ask the system to allocate the resources and
    // that only happens in addToSystem. Furthermore, addToSystem may not alter
    // the Component in any way that would effect its behavior- that is why it
    // it const!  The setting of the variable indices is not in the public
    // interface and is not polymorphic.
