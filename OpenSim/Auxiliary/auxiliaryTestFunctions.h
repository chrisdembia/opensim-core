...
template <typename T>
void ASSERT_EQUAL(T expected, T found, T tolerance, std::string file="",
        int line=-1, std::string message="") {
	if (found < expected - tolerance || found > expected + tolerance)
		throw OpenSim::Exception(message, file, line);
}

void ASSERT(bool cond, std::string file="", int line=-1,
...
void CHECK_STORAGE_AGAINST_STANDARD(OpenSim::Storage& result,
        ...
        OpenSim::Array<double> tolerances, std::string testFile,
        int testFileLine, std::string errorMessage);
