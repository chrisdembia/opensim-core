...
using SimTK::Vec3; using SimTK::Inertia; using SimTK::Transform;
using SimTK::Pi;
...
int main() {
    ...
    Body& humerus = model.add<Body>("humerus", 1, Vec3(0), Inertia(0));
    Body& radius = model.add<Body>("radius", 1, Vec3(0), Inertia(0));

    // Connect the bodies with pin joints. Assume each body is 1 m long.
    PinJoint& shoulder = model.add<PinJoint>("shoulder",
            // Parent body, location in parent, orientation in parent.
            model.getGround(), Vec3(0), Vec3(0),
            // Child body, location in child, orientation in child.
            humerus, Vec3(0, 1, 0), Vec3(0));
    PinJoint& elbow = model.add<PinJoint>("elbow",
            humerus, Vec3(0), Vec3(0), radius, Vec3(0, 1, 0), Vec3(0));

    // Add a muscle that flexes the elbow.
    auto& biceps = model.add<Millard2012EquilibriumMuscle>(
            "biceps", 200, 0.6, 0.55, 0);
    biceps.addNewPathPoint("origin",    humerus, Vec3(0, 0.8, 0));
    biceps.addNewPathPoint("insertion", radius,  Vec3(0, 0.7, 0));

    // Add a controller that specifies the excitation of the muscle.
    PrescribedController& brain = model.add<PrescribedController>();
    ...
    ConsoleReporter& reporter = model.add<ConsoleReporter>();
    ...
    PhysicalOffsetFrame& humerusCenter = humerus.add<PhysicalOffsetFrame>(
            "humerusCenter", "humerus", Transform(Vec3(0, 0.5, 0)));
    humerusCenter.attachGeometry(bodyGeometry.clone());
    PhysicalOffsetFrame& radiusCenter = radius.add<PhysicalOffsetFrame>(
