...
class TestSimbody(unittest.TestCase):
    ...
    def test_vec3_list_constructor(self):
        # We can construct a Vec3 using a python list.
        v1 = osim.Vec3([5, 4, 2])
        assert v1.tolist() == [5, 4, 2]

        v3 = osim.Vec3([5, 4, 5])
        assert v3.tolist() != [5, 4, 2]
    ...
    def test_vector_list_constructor(self):
        v1 = osim.Vector([])
        assert v1.size() == 0
        assert v1.tolist() == []
        v2 = osim.Vector([5])
        assert v2.size() == 1
        assert v2.tolist() == [5]
        v3 = osim.Vector([6, 10, 1])
        assert v3.size() == 3
        assert v3.tolist() == [6, 10, 1]
